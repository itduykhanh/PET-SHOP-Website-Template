...
//                        lưu thông tin vào bảng order_detail{
                    {
                        ...
                        $product=$product_model->detail($key);
                        if($product['id'] == $key){
                            $total_quality=0;
    ...
    {
        $this->content=$this->render('mvc/frontend/views/payments/Mail.php');
...
//    nhập tài khoản gmail cho username
            $mail->Username   = 'shopdunk76thaiha@gmail.com';                     // SMTP username
...
//    password này sẽ đọc lập với password của bạn
            $mail->Password   = 'hrlhzhoamfrkxkvp';                               // SMTP password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;         // Enable TLS encryption; `PHPMailer::ENCRYPTION_SMTPS` encouraged
            $mail->Port       = 587;                                    // TCP port to connect to, use 465 for `PHPMailer::ENCRYPTION_SMTPS` above

            //Recipients
            $mail->setFrom('phananhduc2311@gmail.com', 'ShopDunk');
//    gửi tới ai
            $mail->addAddress($email);     // Add a recipient
            // Content
            $mail->isHTML(true);
            $mail->Subject = 'Cảm ơn bạn đã mua hàng tại cửa hàng điện thoại ShopDunk';
...
//
    ...
    {
        $this->content=$this->render("mvc/frontend/views/payments/thanks.php");
        require_once 'mvc/frontend/views/layouts/main.php';
    }
