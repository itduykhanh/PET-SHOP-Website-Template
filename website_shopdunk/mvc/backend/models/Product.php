...
//    search
    ...
    {
        $from=($page-1) * $pageSize;
        $sql_query ="select products.*,categories.name as category_name from products 
                      inner join categories on products.category_id=categories.id 
...
//    thêm mới sản phẩm
    public function insert()
    {
        $obj_insert = $this->connection
            ->prepare("INSERT INTO products(category_id ,title,ram,capacity, avatar,product_year,present,
                        price , quality , discount ,summary, content, status , hotproduct) 
                                VALUES (:category_id ,:title,:ram,:capacity, :avatar, :product_year,:present,:price, :quality ,
...
//    update sản phẩm
    public function update($id)
    {
        $obj_update = $this->connection
            ->prepare("UPDATE products SET category_id=:category_id ,title=:title,present=:present,
                                quality=:quality ,ram=:ram,capacity=:capacity, discount=:discount,product_year=:product_year ,avatar=:avatar, price=:price,
...
////xóa sản phẩm
    ...
    public function ProductOut(){
        $sql_select= "select products.*,categories.name as category_name from products 
                      inner join categories on products.category_id=categories.id
                    WHERE products.STATUS=1 and products.quality<=0";
        $obj_select=$this->connection->prepare($sql_select);
        $obj_select->execute();
